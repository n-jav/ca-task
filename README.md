...
The same configuration paramters apply as described earlier in the document, only the MODE attribute is changed to "AWS". Once the device simulator application is running, you can navigate to the Kibana access address provided in the document to see the logs being generated by the device simulator.
